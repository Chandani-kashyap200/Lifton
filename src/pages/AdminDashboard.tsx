...
import { Users, Car, Package, TrendingUp, CheckCircle, XCircle, Clock } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const fetchPendingDrivers = async () => {
    const { data } = await supabase.from('drivers').select('*, profiles(full_name, email)').eq('status', 'pending');
  ...
  const fetchAllDrivers = async () => {
    const { data } = await supabase.from('drivers').select('*, profiles(full_name, email)').order('created_at', { ascending: false });
    setAllDrivers(data || []);
  };
